...
K_TREES = 40         # start small (10–40) for a tile
...
MIN_RZ = 1.5
MAX_RZ = 22.0
RESP_BETA = 20.0         # sharpness of competition (try 3..15)
RESP_EPS  = 1e-12
...
class MultiTreeSpheroid(nn.Module):
    ...
    # Broadcast shapes: points -> (N,1,3), mu -> (1,K,3)
     p  = points[:, None, :]   # (N,K,3)
    ...
    # Optical depth above point: dz_above inside crown along vertical line at (x,y)
    ...
    # Shared occlusion from all trees
     optical_depth = torch.sum(lam[None, :] * dz_above, dim=1)  # (N,)

    # Per-tree hit intensity
     hit_i = lam[None, :] * w  # (N,K)

    # Responsibilities (competition)
     log_hit = torch.log(hit_i + RESP_EPS)
     resp = torch.softmax(RESP_BETA * log_hit, dim=1)  # (N,K)

    # Mixture hit term: each point mainly explained by one tree
     hit_mixture = torch.sum(resp * hit_i, dim=1)  # (N,)

     likelihood = hit_mixture * torch.exp(-optical_depth)
     return likelihood + EPS, (rxy, rz, lam,resp)
...
def main():
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        lik, (rxy, rz, lam, resp) = model(points_t)
        with torch.no_grad():
          ent = -(resp * torch.log(resp + 1e-12)).sum(dim=1).mean().item()
        ...
        if epoch % 50 == 0 or epoch == EPOCHS - 1:
            with torch.no_grad():
               print(
                  ...
                  f"rxy(mean)={rxy.mean().item():.2f} rz(mean)={rz.mean().item():.2f} "
                  f"lam(mean)={lam.mean().item():.2f} | resp_entropy={ent:.2f}"
    )
